...
API 配置（可选）
如需 AI 功能，编辑js/config.js替换为您的 DeepSeek API 密钥。
技术架构
纯前端实现（HTML/CSS/JS），可配置后端服务器。
可选 AI 集成：使用 DeepSeek API 实现智能提示。
案例库本地存储于 JSON 文件。

使用流程
训练模式
输入行业关键词（如 "教育"）。
AI 生成反共识问题（如 "教室必须有黑板吗？"）。
提交想法并获取 AI 评估反馈。
案例学习
浏览分类案例（科技 / 消费 / 硬件等）。
查看每个案例的「破界方向」和「情绪共鸣点」。
部署方式
支持静态托管于：
GitHub Pages
Vercel/Netlify
...
许可证
MIT 许可证
🗡️ 核心价值：通过反共识训练与情绪洞察，将创新思维从偶然灵感转化为可训练的能力。
